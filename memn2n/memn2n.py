...
class MemN2N(object):
    ...
    def __init__(self, batch_size, vocab_size, sentence_size, memory_size, embedding_size,
        ...
        correct_prediction = tf.equal(predict_op, tf.argmax(tf.cast(self._answers, tf.float32), 1))
    ...
    def batch_fit(self, stories, queries, answers, learning_rate, merged):
        ...
        loss, _, merged = self._sess.run([self.loss_op, self.train_op, merged], feed_dict=feed_dict)
        return loss, merged

    def predict(self, stories, queries, answers, merged):
        ...
        return self._sess.run([self.predict_op, merged], feed_dict=feed_dict)
